// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const mwi = Country(
    id: 131,
    name: 'Malawi',
    iso3: 'MWI',
    iso2: 'MW',
    phoneCode: '265',
    capital: 'Lilongwe',
    currency: 'MWK',
    currencySymbol: r'MK',
    tld: '.mw',
    native: 'Malawi',
    region: 'Africa',
    subregion: 'Eastern Africa',
    timezones: [
      Timezone(
          zoneName: 'Africa/Blantyre',
          gmtOffset: 7200,
          gmtOffsetName: 'UTC+02:00',
          abbreviation: 'CAT',
          tzName: 'Central Africa Time')
    ],
    emoji: '🇲🇼',
    emojiU: 'U+1F1F2 U+1F1FC',
    states: [
      State(id: 3096, name: 'Balaka District', stateCode: 'BA', cities: []),
      State(id: 3102, name: 'Blantyre District', stateCode: 'BL', cities: []),
      State(id: 3092, name: 'Central Region', stateCode: 'C', cities: [
        City(
            id: 67950,
            name: 'Chipoka',
            latitude: '-13.99329000',
            longitude: '34.51566000'),
        City(
            id: 67955,
            name: 'Dedza',
            latitude: '-14.37790000',
            longitude: '34.33322000'),
        City(
            id: 67956,
            name: 'Dedza District',
            latitude: '-14.26273000',
            longitude: '34.18559000'),
        City(
            id: 67957,
            name: 'Dowa',
            latitude: '-13.65399000',
            longitude: '33.93754000'),
        City(
            id: 67958,
            name: 'Dowa District',
            latitude: '-13.60098000',
            longitude: '33.82378000'),
        City(
            id: 67961,
            name: 'Kasungu',
            latitude: '-13.03333000',
            longitude: '33.48333000'),
        City(
            id: 67962,
            name: 'Kasungu District',
            latitude: '-13.00000000',
            longitude: '33.41667000'),
        City(
            id: 67964,
            name: 'Lilongwe',
            latitude: '-13.96692000',
            longitude: '33.78725000'),
        City(
            id: 67965,
            name: 'Lilongwe District',
            latitude: '-14.01962000',
            longitude: '33.68407000'),
        City(
            id: 67973,
            name: 'Mchinji',
            latitude: '-13.79841000',
            longitude: '32.88019000'),
        City(
            id: 67974,
            name: 'Mchinji District',
            latitude: '-13.76230000',
            longitude: '33.07998000'),
        City(
            id: 67976,
            name: 'Mponela',
            latitude: '-13.53194000',
            longitude: '33.74008000'),
        City(
            id: 67987,
            name: 'Nkhotakota',
            latitude: '-12.92744000',
            longitude: '34.29614000'),
        City(
            id: 67988,
            name: 'Nkhotakota District',
            latitude: '-12.80417000',
            longitude: '34.08298000'),
        City(
            id: 67991,
            name: 'Ntcheu',
            latitude: '-14.82027000',
            longitude: '34.63586000'),
        City(
            id: 67992,
            name: 'Ntcheu District',
            latitude: '-14.75000000',
            longitude: '34.75000000'),
        City(
            id: 67993,
            name: 'Ntchisi',
            latitude: '-13.52775000',
            longitude: '33.91490000'),
        City(
            id: 67994,
            name: 'Ntchisi District',
            latitude: '-13.29574000',
            longitude: '33.90798000'),
        City(
            id: 67999,
            name: 'Salima',
            latitude: '-13.78040000',
            longitude: '34.45870000'),
        City(
            id: 68000,
            name: 'Salima District',
            latitude: '-13.71959000',
            longitude: '34.40184000')
      ]),
      State(id: 3107, name: 'Chikwawa District', stateCode: 'CK', cities: []),
      State(id: 3109, name: 'Chiradzulu District', stateCode: 'CR', cities: []),
      State(id: 3087, name: 'Chitipa district', stateCode: 'CT', cities: []),
      State(id: 3097, name: 'Dedza District', stateCode: 'DE', cities: []),
      State(id: 3090, name: 'Dowa District', stateCode: 'DO', cities: []),
      State(id: 3091, name: 'Karonga District', stateCode: 'KR', cities: []),
      State(id: 3094, name: 'Kasungu District', stateCode: 'KS', cities: []),
      State(id: 3093, name: 'Likoma District', stateCode: 'LK', cities: []),
      State(id: 3101, name: 'Lilongwe District', stateCode: 'LI', cities: []),
      State(id: 3082, name: 'Machinga District', stateCode: 'MH', cities: []),
      State(id: 3110, name: 'Mangochi District', stateCode: 'MG', cities: []),
      State(id: 3099, name: 'Mchinji District', stateCode: 'MC', cities: []),
      State(id: 3103, name: 'Mulanje District', stateCode: 'MU', cities: []),
      State(id: 3084, name: 'Mwanza District', stateCode: 'MW', cities: []),
      State(id: 3104, name: 'Mzimba District', stateCode: 'MZ', cities: []),
      State(id: 3095, name: 'Nkhata Bay District', stateCode: 'NB', cities: []),
      State(id: 3100, name: 'Nkhotakota District', stateCode: 'NK', cities: []),
      State(id: 3105, name: 'Northern Region', stateCode: 'N', cities: [
        City(
            id: 67953,
            name: 'Chitipa',
            latitude: '-9.70237000',
            longitude: '33.26969000'),
        City(
            id: 67954,
            name: 'Chitipa District',
            latitude: '-9.92727000',
            longitude: '33.42541000'),
        City(
            id: 67959,
            name: 'Karonga',
            latitude: '-9.93333000',
            longitude: '33.93333000'),
        City(
            id: 67960,
            name: 'Karonga District',
            latitude: '-10.11153000',
            longitude: '33.88151000'),
        City(
            id: 67963,
            name: 'Likoma District',
            latitude: '-12.06065000',
            longitude: '34.73608000'),
        City(
            id: 67966,
            name: 'Livingstonia',
            latitude: '-10.60602000',
            longitude: '34.10628000'),
        City(
            id: 67981,
            name: 'Mzimba',
            latitude: '-11.90000000',
            longitude: '33.60000000'),
        City(
            id: 67982,
            name: 'Mzimba District',
            latitude: '-11.85428000',
            longitude: '33.60778000'),
        City(
            id: 67983,
            name: 'Mzuzu',
            latitude: '-11.46556000',
            longitude: '34.02071000'),
        City(
            id: 67985,
            name: 'Nkhata Bay',
            latitude: '-11.60659000',
            longitude: '34.29073000'),
        City(
            id: 67986,
            name: 'Nkhata Bay District',
            latitude: '-11.65304000',
            longitude: '34.05322000'),
        City(
            id: 67997,
            name: 'Rumphi',
            latitude: '-11.01863000',
            longitude: '33.85746000'),
        City(
            id: 67998,
            name: 'Rumphi District',
            latitude: '-10.80292000',
            longitude: '33.80524000')
      ]),
      State(id: 3085, name: 'Nsanje District', stateCode: 'NS', cities: []),
      State(id: 3088, name: 'Ntcheu District', stateCode: 'NU', cities: []),
      State(id: 3111, name: 'Ntchisi District', stateCode: 'NI', cities: []),
      State(id: 3108, name: 'Phalombe District', stateCode: 'PH', cities: []),
      State(id: 3089, name: 'Rumphi District', stateCode: 'RU', cities: []),
      State(id: 3086, name: 'Salima District', stateCode: 'SA', cities: []),
      State(id: 3106, name: 'Southern Region', stateCode: 'S', cities: [
        City(
            id: 67944,
            name: 'Balaka',
            latitude: '-14.97928000',
            longitude: '34.95575000'),
        City(
            id: 67945,
            name: 'Balaka District',
            latitude: '-15.04839000',
            longitude: '35.05910000'),
        City(
            id: 67946,
            name: 'Blantyre',
            latitude: '-15.78499000',
            longitude: '35.00854000'),
        City(
            id: 67947,
            name: 'Blantyre District',
            latitude: '-15.64732000',
            longitude: '34.93956000'),
        City(
            id: 67948,
            name: 'Chikwawa',
            latitude: '-16.03352000',
            longitude: '34.80091000'),
        City(
            id: 67949,
            name: 'Chikwawa District',
            latitude: '-16.16667000',
            longitude: '34.75000000'),
        City(
            id: 67951,
            name: 'Chiradzulu',
            latitude: '-15.67461000',
            longitude: '35.14071000'),
        City(
            id: 67952,
            name: 'Chiradzulu District',
            latitude: '-15.75268000',
            longitude: '35.21576000'),
        City(
            id: 67967,
            name: 'Liwonde',
            latitude: '-15.06665000',
            longitude: '35.22543000'),
        City(
            id: 67968,
            name: 'Luchenza',
            latitude: '-16.00693000',
            longitude: '35.30947000'),
        City(
            id: 67969,
            name: 'Machinga',
            latitude: '-15.16849000',
            longitude: '35.30002000'),
        City(
            id: 67970,
            name: 'Machinga District',
            latitude: '-14.94583000',
            longitude: '35.57367000'),
        City(
            id: 67971,
            name: 'Mangochi',
            latitude: '-14.47815000',
            longitude: '35.26448000'),
        City(
            id: 67972,
            name: 'Mangochi District',
            latitude: '-14.39296000',
            longitude: '35.34658000'),
        City(
            id: 67975,
            name: 'Monkey Bay',
            latitude: '-14.08239000',
            longitude: '34.91652000'),
        City(
            id: 67977,
            name: 'Mulanje',
            latitude: '-16.03163000',
            longitude: '35.50000000'),
        City(
            id: 67978,
            name: 'Mulanje District',
            latitude: '-15.93440000',
            longitude: '35.49990000'),
        City(
            id: 67979,
            name: 'Mwanza',
            latitude: '-15.60262000',
            longitude: '34.52479000'),
        City(
            id: 67980,
            name: 'Mwanza District',
            latitude: '-15.63337000',
            longitude: '34.51682000'),
        City(
            id: 67984,
            name: 'Neno District',
            latitude: '-15.45051000',
            longitude: '34.70034000'),
        City(
            id: 67989,
            name: 'Nsanje',
            latitude: '-16.91995000',
            longitude: '35.26199000'),
        City(
            id: 67990,
            name: 'Nsanje District',
            latitude: '-16.75000000',
            longitude: '35.16667000'),
        City(
            id: 67995,
            name: 'Phalombe',
            latitude: '-15.80635000',
            longitude: '35.65067000'),
        City(
            id: 67996,
            name: 'Phalombe District',
            latitude: '-15.66667000',
            longitude: '35.66667000'),
        City(
            id: 68001,
            name: 'Thyolo',
            latitude: '-16.06775000',
            longitude: '35.14046000'),
        City(
            id: 68002,
            name: 'Thyolo District',
            latitude: '-16.10583000',
            longitude: '35.15060000'),
        City(
            id: 68003,
            name: 'Zomba',
            latitude: '-15.38596000',
            longitude: '35.31880000'),
        City(
            id: 68004,
            name: 'Zomba District',
            latitude: '-15.43007000',
            longitude: '35.41829000')
      ]),
      State(id: 3098, name: 'Thyolo District', stateCode: 'TH', cities: []),
      State(id: 3083, name: 'Zomba District', stateCode: 'ZO', cities: [])
    ]);
