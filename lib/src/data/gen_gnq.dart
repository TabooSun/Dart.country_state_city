// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const gnq = Country(
    id: 67,
    name: 'Equatorial Guinea',
    iso3: 'GNQ',
    iso2: 'GQ',
    phoneCode: '240',
    capital: 'Malabo',
    currency: 'XAF',
    currencySymbol: r'FCFA',
    tld: '.gq',
    native: 'Guinea Ecuatorial',
    region: 'Africa',
    subregion: 'Middle Africa',
    timezones: [
      Timezone(
          zoneName: 'Africa/Malabo',
          gmtOffset: 3600,
          gmtOffsetName: 'UTC+01:00',
          abbreviation: 'WAT',
          tzName: 'West Africa Time')
    ],
    emoji: '🇬🇶',
    emojiU: 'U+1F1EC U+1F1F6',
    states: [
      State(id: 3444, name: 'Annobón Province', stateCode: 'AN', cities: [
        City(
            id: 52397,
            name: 'San Antonio de Palé',
            latitude: '-1.40680000',
            longitude: '5.63178000')
      ]),
      State(id: 3446, name: 'Bioko Norte Province', stateCode: 'BN', cities: [
        City(
            id: 52387,
            name: 'Malabo',
            latitude: '3.75578000',
            longitude: '8.78166000'),
        City(
            id: 52395,
            name: 'Rebola',
            latitude: '3.71667000',
            longitude: '8.83333000'),
        City(
            id: 52398,
            name: 'Santiago de Baney',
            latitude: '3.69920000',
            longitude: '8.90840000')
      ]),
      State(id: 3443, name: 'Bioko Sur Province', stateCode: 'BS', cities: [
        City(
            id: 52385,
            name: 'Luba',
            latitude: '3.45683000',
            longitude: '8.55465000')
      ]),
      State(id: 3445, name: 'Centro Sur Province', stateCode: 'CS', cities: [
        City(
            id: 52376,
            name: 'Acurenam',
            latitude: '1.03225000',
            longitude: '10.64882000'),
        City(
            id: 52380,
            name: 'Bicurga',
            latitude: '1.58113000',
            longitude: '10.46716000'),
        City(
            id: 52384,
            name: 'Evinayong',
            latitude: '1.43677000',
            longitude: '10.55124000')
      ]),
      State(id: 3442, name: 'Insular Region', stateCode: 'I', cities: []),
      State(id: 3439, name: 'Kié-Ntem Province', stateCode: 'KN', cities: [
        City(
            id: 52383,
            name: 'Ebebiyin',
            latitude: '2.15106000',
            longitude: '11.33528000'),
        City(
            id: 52390,
            name: 'Mikomeseng',
            latitude: '2.13609000',
            longitude: '10.61322000'),
        City(
            id: 52392,
            name: 'Ncue',
            latitude: '2.01643000',
            longitude: '10.47066000'),
        City(
            id: 52393,
            name: 'Nsang',
            latitude: '2.02475000',
            longitude: '10.94599000')
      ]),
      State(id: 3441, name: 'Litoral Province', stateCode: 'LI', cities: [
        City(
            id: 52379,
            name: 'Bata',
            latitude: '1.86391000',
            longitude: '9.76582000'),
        City(
            id: 52381,
            name: 'Bitica',
            latitude: '1.42610000',
            longitude: '9.62316000'),
        City(
            id: 52382,
            name: 'Cogo',
            latitude: '1.08425000',
            longitude: '9.69300000'),
        City(
            id: 52386,
            name: 'Machinda',
            latitude: '1.88262000',
            longitude: '9.95133000'),
        City(
            id: 52388,
            name: 'Mbini',
            latitude: '1.58267000',
            longitude: '9.61478000'),
        City(
            id: 52396,
            name: 'Río Campo',
            latitude: '2.33812000',
            longitude: '9.82212000')
      ]),
      State(id: 3438, name: 'Río Muni', stateCode: 'C', cities: []),
      State(id: 3440, name: 'Wele-Nzas Province', stateCode: 'WN', cities: [
        City(
            id: 52375,
            name: 'Aconibe',
            latitude: '1.29683000',
            longitude: '10.93691000'),
        City(
            id: 52378,
            name: 'Añisoc',
            latitude: '1.86580000',
            longitude: '10.76892000'),
        City(
            id: 52377,
            name: 'Ayene',
            latitude: '1.85592000',
            longitude: '10.68994000'),
        City(
            id: 52389,
            name: 'Mengomeyén',
            latitude: '1.69439000',
            longitude: '11.03422000'),
        City(
            id: 52391,
            name: 'Mongomo',
            latitude: '1.62742000',
            longitude: '11.31346000'),
        City(
            id: 52394,
            name: 'Nsok',
            latitude: '1.12985000',
            longitude: '11.26603000')
      ])
    ]);
