// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const tun = Country(
    id: 224,
    name: 'Tunisia',
    iso3: 'TUN',
    iso2: 'TN',
    phoneCode: '216',
    capital: 'Tunis',
    currency: 'TND',
    currencySymbol: r'ت.د',
    tld: '.tn',
    native: 'تونس',
    region: 'Africa',
    subregion: 'Northern Africa',
    timezones: [
      Timezone(
          zoneName: 'Africa/Tunis',
          gmtOffset: 3600,
          gmtOffsetName: 'UTC+01:00',
          abbreviation: 'CET',
          tzName: 'Central European Time')
    ],
    emoji: '🇹🇳',
    emojiU: 'U+1F1F9 U+1F1F3',
    states: [
      State(id: 2550, name: 'Ariana Governorate', stateCode: '12', cities: [
        City(
            id: 106915,
            name: 'Ariana',
            latitude: '36.86012000',
            longitude: '10.19337000'),
        City(
            id: 106955,
            name: 'Galaat el Andeless',
            latitude: '37.06290000',
            longitude: '10.11829000')
      ]),
      State(id: 2566, name: 'Ben Arous Governorate', stateCode: '13', cities: [
        City(
            id: 106920,
            name: 'Ben Arous',
            latitude: '36.75452000',
            longitude: '10.22167000'),
        City(
            id: 106959,
            name: 'Hammam Lif',
            latitude: '36.71919000',
            longitude: '10.32233000'),
        City(
            id: 106961,
            name: 'Hammam-Lif',
            latitude: '36.72866000',
            longitude: '10.34163000'),
        City(
            id: 106977,
            name: 'La Sebala du Mornag',
            latitude: '36.67931000',
            longitude: '10.29195000'),
        City(
            id: 107007,
            name: 'Radès',
            latitude: '36.76946000',
            longitude: '10.27468000')
      ]),
      State(id: 2551, name: 'Bizerte Governorate', stateCode: '23', cities: [
        City(
            id: 106913,
            name: 'Al Matlīn',
            latitude: '37.24516000',
            longitude: '10.05000000'),
        City(
            id: 106926,
            name: 'Bizerte',
            latitude: '37.27442000',
            longitude: '9.87391000'),
        City(
            id: 106927,
            name: 'Bizerte Sud',
            latitude: '37.25528000',
            longitude: '9.67915000'),
        City(
            id: 106938,
            name: 'Douar Tindja',
            latitude: '37.16667000',
            longitude: '9.75000000'),
        City(
            id: 106941,
            name: 'El Alia',
            latitude: '37.16911000',
            longitude: '10.03478000'),
        City(
            id: 106983,
            name: 'Mateur',
            latitude: '37.04045000',
            longitude: '9.66557000'),
        City(
            id: 106988,
            name: 'Menzel Abderhaman',
            latitude: '37.23737000',
            longitude: '9.86313000'),
        City(
            id: 106989,
            name: 'Menzel Bourguiba',
            latitude: '37.15368000',
            longitude: '9.78594000'),
        City(
            id: 106990,
            name: 'Menzel Jemil',
            latitude: '37.23618000',
            longitude: '9.91448000'),
        City(
            id: 107008,
            name: 'Rafrāf',
            latitude: '37.19043000',
            longitude: '10.18365000'),
        City(
            id: 107010,
            name: 'Rhar el Melah',
            latitude: '37.16939000',
            longitude: '10.19064000'),
        City(
            id: 107017,
            name: 'Sejenane',
            latitude: '37.05722000',
            longitude: '9.23806000'),
        City(
            id: 107041,
            name: 'Zahānah',
            latitude: '37.03959000',
            longitude: '10.03876000')
      ]),
      State(id: 2558, name: 'Gabès Governorate', stateCode: '81', cities: [
        City(
            id: 106929,
            name: 'Bou Attouche',
            latitude: '33.89927000',
            longitude: '9.78496000'),
        City(
            id: 106945,
            name: 'El Hamma',
            latitude: '33.89152000',
            longitude: '9.79629000'),
        City(
            id: 106952,
            name: 'Gabès',
            latitude: '33.88146000',
            longitude: '10.09820000'),
        City(
            id: 106984,
            name: 'Matmata',
            latitude: '33.54445000',
            longitude: '9.97157000')
      ]),
      State(id: 2556, name: 'Gafsa Governorate', stateCode: '71', cities: [
        City(
            id: 106914,
            name: 'Ar Rudayyif',
            latitude: '34.38270000',
            longitude: '8.15549000'),
        City(
            id: 106916,
            name: 'As Sanad',
            latitude: '34.46280000',
            longitude: '9.26404000'),
        City(
            id: 106954,
            name: 'Gafsa',
            latitude: '34.42500000',
            longitude: '8.78417000'),
        City(
            id: 106994,
            name: 'Metlaoui',
            latitude: '34.32081000',
            longitude: '8.40157000'),
        City(
            id: 107000,
            name: 'Mu‘tamadīyat ar Rudayyif',
            latitude: '34.40081000',
            longitude: '8.17057000')
      ]),
      State(id: 2552, name: 'Jendouba Governorate', stateCode: '32', cities: [
        City(
            id: 106951,
            name: 'Fernana',
            latitude: '36.65547000',
            longitude: '8.69602000'),
        City(
            id: 106965,
            name: 'Jendouba',
            latitude: '36.48519000',
            longitude: '8.82325000'),
        City(
            id: 107005,
            name: 'Oued Meliz',
            latitude: '36.46813000',
            longitude: '8.54951000'),
        City(
            id: 107030,
            name: 'Tabarka',
            latitude: '36.95442000',
            longitude: '8.75801000')
      ]),
      State(id: 2564, name: 'Kairouan Governorate', stateCode: '41', cities: [
        City(
            id: 106958,
            name: 'Haffouz',
            latitude: '35.63235000',
            longitude: '9.67624000'),
        City(
            id: 106968,
            name: 'Kairouan',
            latitude: '35.67810000',
            longitude: '10.09633000'),
        City(
            id: 107016,
            name: 'Sbikha',
            latitude: '35.93325000',
            longitude: '10.02081000')
      ]),
      State(id: 2570, name: 'Kasserine Governorate', stateCode: '42', cities: [
        City(
            id: 106969,
            name: 'Kasserine',
            latitude: '35.16758000',
            longitude: '8.83651000'),
        City(
            id: 107011,
            name: 'Rohia',
            latitude: '35.65129000',
            longitude: '9.05306000'),
        City(
            id: 107015,
            name: 'Sbiba',
            latitude: '35.54332000',
            longitude: '9.07370000'),
        City(
            id: 107036,
            name: 'Thala',
            latitude: '35.57244000',
            longitude: '8.67031000')
      ]),
      State(id: 2572, name: 'Kassrine', stateCode: '31', cities: [
        City(
            id: 106930,
            name: 'Béja',
            latitude: '36.72564000',
            longitude: '9.18169000'),
        City(
            id: 106940,
            name: 'Délégation de Béja Nord',
            latitude: '36.80353000',
            longitude: '9.24984000'),
        City(
            id: 106956,
            name: 'Goubellat',
            latitude: '36.54178000',
            longitude: '9.66334000'),
        City(
            id: 106986,
            name: 'Medjez el Bab',
            latitude: '36.64964000',
            longitude: '9.61231000'),
        City(
            id: 107031,
            name: 'Tabursuq',
            latitude: '36.45692000',
            longitude: '9.24751000'),
        City(
            id: 107035,
            name: 'Testour',
            latitude: '36.55130000',
            longitude: '9.44307000')
      ]),
      State(id: 2562, name: 'Kebili Governorate', stateCode: '73', cities: [
        City(
            id: 106939,
            name: 'Douz',
            latitude: '33.46632000',
            longitude: '9.02030000'),
        City(
            id: 106944,
            name: 'El Golaa',
            latitude: '33.48485000',
            longitude: '9.00678000'),
        City(
            id: 106964,
            name: 'Jemna',
            latitude: '33.57778000',
            longitude: '9.01472000'),
        City(
            id: 106970,
            name: 'Kebili',
            latitude: '33.70439000',
            longitude: '8.96903000')
      ]),
      State(id: 2561, name: 'Kef Governorate', stateCode: '33', cities: [
        City(
            id: 106917,
            name: 'As Sars',
            latitude: '36.07640000',
            longitude: '9.02117000'),
        City(
            id: 106947,
            name: 'El Kef',
            latitude: '36.17424000',
            longitude: '8.70486000'),
        City(
            id: 106948,
            name: 'El Ksour',
            latitude: '35.89607000',
            longitude: '8.88493000'),
        City(
            id: 106992,
            name: 'Menzel Salem',
            latitude: '35.85673000',
            longitude: '8.47654000'),
        City(
            id: 107002,
            name: 'Nibbar',
            latitude: '36.29411000',
            longitude: '8.76657000'),
        City(
            id: 107013,
            name: 'Sakiet Sidi Youssef',
            latitude: '36.22292000',
            longitude: '8.35547000'),
        City(
            id: 107032,
            name: 'Tajerouine',
            latitude: '35.89174000',
            longitude: '8.55276000')
      ]),
      State(id: 2568, name: 'Mahdia Governorate', stateCode: '53', cities: [
        City(
            id: 106932,
            name: 'Chebba',
            latitude: '35.23722000',
            longitude: '11.11500000'),
        City(
            id: 106934,
            name: 'Chorbane',
            latitude: '35.28581000',
            longitude: '10.38580000'),
        City(
            id: 106946,
            name: 'El Jem',
            latitude: '35.30000000',
            longitude: '10.71667000'),
        City(
            id: 106974,
            name: 'Ksour Essaf',
            latitude: '35.41808000',
            longitude: '10.99475000'),
        City(
            id: 106980,
            name: 'Mahdia',
            latitude: '35.50472000',
            longitude: '11.06222000'),
        City(
            id: 106987,
            name: 'Melloulèche',
            latitude: '35.16617000',
            longitude: '11.03504000'),
        City(
            id: 107014,
            name: 'Salakta',
            latitude: '35.39444000',
            longitude: '11.04361000'),
        City(
            id: 107020,
            name: 'Sidi Alouane',
            latitude: '35.37505000',
            longitude: '10.93899000'),
        City(
            id: 107043,
            name: 'Zouila',
            latitude: '35.50056000',
            longitude: '11.06056000')
      ]),
      State(id: 2555, name: 'Manouba Governorate', stateCode: '14', cities: [
        City(
            id: 106942,
            name: 'El Battan',
            latitude: '36.80368000',
            longitude: '9.84424000'),
        City(
            id: 106982,
            name: 'Manouba',
            latitude: '36.81006000',
            longitude: '10.09557000'),
        City(
            id: 106999,
            name: 'Mu‘tamadīyat Manūbah',
            latitude: '36.80907000',
            longitude: '10.09467000'),
        City(
            id: 107004,
            name: 'Oued Lill',
            latitude: '36.83408000',
            longitude: '10.04057000')
      ]),
      State(id: 2560, name: 'Medenine Governorate', stateCode: '82', cities: [
        City(
            id: 106921,
            name: 'Ben Gardane',
            latitude: '33.13783000',
            longitude: '11.21965000'),
        City(
            id: 106923,
            name: 'Beni Kheddache',
            latitude: '33.25279000',
            longitude: '10.19883000'),
        City(
            id: 106950,
            name: 'Erriadh',
            latitude: '33.82063000',
            longitude: '10.85394000'),
        City(
            id: 106963,
            name: 'Houmt El Souk',
            latitude: '33.87576000',
            longitude: '10.85745000'),
        City(
            id: 106966,
            name: 'Jerba Midoun',
            latitude: '33.77918000',
            longitude: '10.95215000'),
        City(
            id: 106985,
            name: 'Medenine',
            latitude: '33.35495000',
            longitude: '10.50548000'),
        City(
            id: 106996,
            name: 'Midoun',
            latitude: '33.80813000',
            longitude: '10.99228000'),
        City(
            id: 107042,
            name: 'Zarzis',
            latitude: '33.50398000',
            longitude: '11.11215000')
      ]),
      State(id: 2553, name: 'Monastir Governorate', stateCode: '52', cities: [
        City(
            id: 106918,
            name: 'Banbalah',
            latitude: '35.70000000',
            longitude: '10.80000000'),
        City(
            id: 106919,
            name: 'Bekalta',
            latitude: '35.61739000',
            longitude: '10.99466000'),
        City(
            id: 106922,
            name: 'Beni Hassane',
            latitude: '35.56720000',
            longitude: '10.80869000'),
        City(
            id: 106937,
            name: 'Djemmal',
            latitude: '35.62231000',
            longitude: '10.75696000'),
        City(
            id: 106972,
            name: 'Ksar Hellal',
            latitude: '35.64773000',
            longitude: '10.89046000'),
        City(
            id: 106973,
            name: 'Ksibet el Mediouni',
            latitude: '35.68561000',
            longitude: '10.84256000'),
        City(
            id: 106979,
            name: 'Lemta',
            latitude: '35.66667000',
            longitude: '10.88333000'),
        City(
            id: 106991,
            name: 'Menzel Kamel',
            latitude: '35.62477000',
            longitude: '10.66727000'),
        City(
            id: 106993,
            name: 'Mesdour',
            latitude: '35.68206000',
            longitude: '10.72746000'),
        City(
            id: 106997,
            name: 'Monastir',
            latitude: '35.77799000',
            longitude: '10.82617000'),
        City(
            id: 107003,
            name: 'Ouardenine',
            latitude: '35.70915000',
            longitude: '10.67397000'),
        City(
            id: 107012,
            name: 'Sahline',
            latitude: '35.75166000',
            longitude: '10.71109000'),
        City(
            id: 107018,
            name: 'Seïada',
            latitude: '35.66887000',
            longitude: '10.89246000'),
        City(
            id: 107021,
            name: 'Sidi Ben Nour',
            latitude: '35.53333000',
            longitude: '10.91667000'),
        City(
            id: 107027,
            name: 'Skanes',
            latitude: '35.78333000',
            longitude: '10.80000000'),
        City(
            id: 107037,
            name: 'Touza',
            latitude: '35.63544000',
            longitude: '10.82732000')
      ]),
      State(id: 2557, name: 'Sfax Governorate', stateCode: '61', cities: [
        City(
            id: 106910,
            name: 'Agareb',
            latitude: '34.74406000',
            longitude: '10.46110000'),
        City(
            id: 106924,
            name: 'Bir Ali Ben Khalifa',
            latitude: '34.73592000',
            longitude: '10.09240000'),
        City(
            id: 106936,
            name: 'Djebeniana',
            latitude: '35.03500000',
            longitude: '10.90809000'),
        City(
            id: 106957,
            name: 'Gremda',
            latitude: '34.75000000',
            longitude: '10.78333000'),
        City(
            id: 107019,
            name: 'Sfax',
            latitude: '34.74056000',
            longitude: '10.76028000'),
        City(
            id: 107028,
            name: 'Skhira',
            latitude: '34.29920000',
            longitude: '10.06999000')
      ]),
      State(
          id: 2567,
          name: 'Sidi Bouzid Governorate',
          stateCode: '43',
          cities: [
            City(
                id: 106925,
                name: 'Bir el Hafey',
                latitude: '34.93212000',
                longitude: '9.19321000'),
            City(
                id: 106949,
                name: 'Er Regueb',
                latitude: '34.85932000',
                longitude: '9.78654000'),
            City(
                id: 106967,
                name: 'Jilma',
                latitude: '35.27311000',
                longitude: '9.42385000'),
            City(
                id: 106995,
                name: 'Mezzouna',
                latitude: '34.57758000',
                longitude: '9.84193000'),
            City(
                id: 107024,
                name: 'Sidi Bouzid',
                latitude: '35.03823000',
                longitude: '9.48494000')
          ]),
      State(id: 2563, name: 'Siliana Governorate', stateCode: '34', cities: [
        City(
            id: 106928,
            name: 'Bou Arada',
            latitude: '36.35251000',
            longitude: '9.62175000'),
        City(
            id: 106953,
            name: 'Gafour',
            latitude: '36.32045000',
            longitude: '9.32424000'),
        City(
            id: 106971,
            name: 'Kesra',
            latitude: '35.81363000',
            longitude: '9.36434000'),
        City(
            id: 106978,
            name: 'Le Krib',
            latitude: '36.32802000',
            longitude: '9.13613000'),
        City(
            id: 106981,
            name: 'Maktar',
            latitude: '35.85798000',
            longitude: '9.20072000'),
        City(
            id: 107026,
            name: 'Siliana',
            latitude: '36.08497000',
            longitude: '9.37082000')
      ]),
      State(id: 2571, name: 'Sousse Governorate', stateCode: '51', cities: [
        City(
            id: 106911,
            name: 'Akouda',
            latitude: '35.86910000',
            longitude: '10.56530000'),
        City(
            id: 106960,
            name: 'Hammam Sousse',
            latitude: '35.86090000',
            longitude: '10.60313000'),
        City(
            id: 106962,
            name: 'Harqalah',
            latitude: '36.03027000',
            longitude: '10.50904000'),
        City(
            id: 106998,
            name: 'Msaken',
            latitude: '35.72917000',
            longitude: '10.58082000'),
        City(
            id: 107006,
            name: 'Port el Kantaoui',
            latitude: '35.89239000',
            longitude: '10.59434000'),
        City(
            id: 107022,
            name: 'Sidi Bou Ali',
            latitude: '35.95667000',
            longitude: '10.47306000'),
        City(
            id: 107025,
            name: 'Sidi el Hani',
            latitude: '35.67139000',
            longitude: '10.31583000'),
        City(
            id: 107029,
            name: 'Sousse',
            latitude: '35.82539000',
            longitude: '10.63699000')
      ]),
      State(id: 2559, name: 'Tataouine Governorate', stateCode: '83', cities: [
        City(
            id: 107009,
            name: 'Remada',
            latitude: '32.31662000',
            longitude: '10.39551000'),
        City(
            id: 107034,
            name: 'Tataouine',
            latitude: '32.92967000',
            longitude: '10.45177000')
      ]),
      State(id: 2569, name: 'Tozeur Governorate', stateCode: '72', cities: [
        City(
            id: 106933,
            name: 'Chebika',
            latitude: '34.31909000',
            longitude: '7.93519000'),
        City(
            id: 106935,
            name: 'Degache',
            latitude: '33.97606000',
            longitude: '8.20810000'),
        City(
            id: 107001,
            name: 'Nefta',
            latitude: '33.87309000',
            longitude: '7.87765000'),
        City(
            id: 107033,
            name: 'Tamaghzah',
            latitude: '34.38849000',
            longitude: '7.94313000'),
        City(
            id: 107038,
            name: 'Tozeur',
            latitude: '33.91968000',
            longitude: '8.13352000')
      ]),
      State(id: 2554, name: 'Tunis Governorate', stateCode: '11', cities: [
        City(
            id: 106912,
            name: 'Al Marsá',
            latitude: '36.87818000',
            longitude: '10.32466000'),
        City(
            id: 106931,
            name: 'Carthage',
            latitude: '36.85961000',
            longitude: '10.32978000'),
        City(
            id: 106975,
            name: 'La Goulette',
            latitude: '36.81825000',
            longitude: '10.30520000'),
        City(
            id: 106976,
            name: 'La Mohammedia',
            latitude: '36.67446000',
            longitude: '10.15633000'),
        City(
            id: 107023,
            name: 'Sidi Bou Saïd',
            latitude: '36.86870000',
            longitude: '10.34174000'),
        City(
            id: 107039,
            name: 'Tunis',
            latitude: '36.81897000',
            longitude: '10.16579000')
      ]),
      State(id: 2565, name: 'Zaghouan Governorate', stateCode: '22', cities: [
        City(
            id: 106943,
            name: 'El Fahs',
            latitude: '36.37419000',
            longitude: '9.90651000'),
        City(
            id: 107040,
            name: 'Zaghouan',
            latitude: '36.40291000',
            longitude: '10.14292000')
      ])
    ]);
