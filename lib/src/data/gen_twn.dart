// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const twn = Country(
    id: 216,
    name: 'Taiwan',
    iso3: 'TWN',
    iso2: 'TW',
    phoneCode: '886',
    capital: 'Taipei',
    currency: 'TWD',
    currencySymbol: r'$',
    tld: '.tw',
    native: '臺灣',
    region: 'Asia',
    subregion: 'Eastern Asia',
    timezones: [
      Timezone(
          zoneName: 'Asia/Taipei',
          gmtOffset: 28800,
          gmtOffsetName: 'UTC+08:00',
          abbreviation: 'CST',
          tzName: 'China Standard Time')
    ],
    emoji: '🇹🇼',
    emojiU: 'U+1F1F9 U+1F1FC',
    states: [
      State(id: 3404, name: 'Changhua County', stateCode: 'CHA', cities: [
        City(
            id: 109003,
            name: 'Changhua',
            latitude: '23.95361000',
            longitude: '120.49083000'),
        City(
            id: 109038,
            name: 'Yuanlin',
            latitude: '23.95671000',
            longitude: '120.57608000')
      ]),
      State(id: 3418, name: 'Chiayi City', stateCode: 'CYQ', cities: [
        City(
            id: 109005,
            name: 'Chiayi County',
            latitude: '23.46333000',
            longitude: '120.58166000')
      ]),
      State(id: 3408, name: 'Chiayi County', stateCode: 'CYI', cities: [
        City(
            id: 109004,
            name: 'Chiayi',
            latitude: '23.47722000',
            longitude: '120.44527000'),
        City(
            id: 109026,
            name: 'Pizitou',
            latitude: '23.48556000',
            longitude: '120.44472000')
      ]),
      State(id: 3417, name: 'Hsinchu', stateCode: 'HSZ', cities: [
        City(
            id: 109011,
            name: 'Hsinchu County',
            latitude: '24.67416000',
            longitude: '121.16111000')
      ]),
      State(id: 3423, name: 'Hsinchu County', stateCode: 'HSQ', cities: [
        City(
            id: 109010,
            name: 'Hsinchu',
            latitude: '24.80361000',
            longitude: '120.96861000')
      ]),
      State(id: 3411, name: 'Hualien County', stateCode: 'HUA', cities: [
        City(
            id: 109012,
            name: 'Hualien',
            latitude: '23.78166000',
            longitude: '121.39333000'),
        City(
            id: 109013,
            name: 'Hualien City',
            latitude: '23.97694000',
            longitude: '121.60444000')
      ]),
      State(id: 3412, name: 'Kaohsiung', stateCode: 'KHH', cities: [
        City(
            id: 109016,
            name: 'Kaohsiung',
            latitude: '22.61626000',
            longitude: '120.31333000')
      ]),
      State(id: 3409, name: 'Kaohsiung County', stateCode: 'KHQ', cities: []),
      State(id: 3415, name: 'Kinmen', stateCode: 'KIN', cities: [
        City(
            id: 109014,
            name: 'Jincheng',
            latitude: '24.43415000',
            longitude: '118.31712000'),
        City(
            id: 109017,
            name: 'Kinmen County',
            latitude: '24.45333000',
            longitude: '118.38861000')
      ]),
      State(id: 3420, name: 'Lienchiang County', stateCode: 'LIE', cities: [
        City(
            id: 109018,
            name: 'Lienchiang',
            latitude: '26.37004000',
            longitude: '120.49545000'),
        City(
            id: 109022,
            name: 'Nangan',
            latitude: '26.15039000',
            longitude: '119.93284000')
      ]),
      State(id: 3413, name: 'Miaoli County', stateCode: 'MIA', cities: [
        City(
            id: 109021,
            name: 'Miaoli',
            latitude: '24.48972000',
            longitude: '120.90638000')
      ]),
      State(id: 3407, name: 'Nantou County', stateCode: 'NAN', cities: [
        City(
            id: 109019,
            name: 'Lugu',
            latitude: '23.74639000',
            longitude: '120.75250000'),
        City(
            id: 109023,
            name: 'Nantou',
            latitude: '23.83419000',
            longitude: '120.92704000'),
        City(
            id: 109027,
            name: 'Puli',
            latitude: '23.96639000',
            longitude: '120.96952000'),
        City(
            id: 109041,
            name: 'Zhongxing New Village',
            latitude: '23.95908000',
            longitude: '120.68516000')
      ]),
      State(id: 3403, name: 'Penghu County', stateCode: 'PEN', cities: [
        City(
            id: 109020,
            name: 'Magong',
            latitude: '23.56540000',
            longitude: '119.58627000'),
        City(
            id: 109024,
            name: 'Penghu County',
            latitude: '23.57111000',
            longitude: '119.61138000')
      ]),
      State(id: 3405, name: 'Pingtung County', stateCode: 'PIF', cities: [
        City(
            id: 109007,
            name: 'Donggang',
            latitude: '22.46515000',
            longitude: '120.44927000'),
        City(
            id: 109009,
            name: 'Hengchun',
            latitude: '22.00417000',
            longitude: '120.74389000'),
        City(
            id: 109025,
            name: 'Pingtung',
            latitude: '22.49555000',
            longitude: '120.61444000')
      ]),
      State(id: 3406, name: 'Taichung', stateCode: 'TXG', cities: [
        City(
            id: 109028,
            name: 'Taichung',
            latitude: '24.14690000',
            longitude: '120.68390000'),
        City(
            id: 109029,
            name: 'Taichung City',
            latitude: '24.15472000',
            longitude: '120.67297000')
      ]),
      State(id: 3414, name: 'Taichung County', stateCode: 'TXQ', cities: []),
      State(id: 3421, name: 'Tainan', stateCode: 'TNN', cities: [
        City(
            id: 109030,
            name: 'Tainan',
            latitude: '22.99083000',
            longitude: '120.21333000'),
        City(
            id: 109039,
            name: 'Yujing',
            latitude: '23.12493000',
            longitude: '120.46138000')
      ]),
      State(id: 3401, name: 'Tainan County', stateCode: 'TNQ', cities: []),
      State(id: 3422, name: 'Taipei', stateCode: 'TPE', cities: [
        City(
            id: 109002,
            name: 'Banqiao',
            latitude: '25.01427000',
            longitude: '121.46719000'),
        City(
            id: 109015,
            name: 'Jiufen',
            latitude: '25.10957000',
            longitude: '121.84424000'),
        City(
            id: 109031,
            name: 'Taipei',
            latitude: '25.04776000',
            longitude: '121.53185000'),
        City(
            id: 109032,
            name: 'Taipei City',
            latitude: '25.08300000',
            longitude: '121.55331000')
      ]),
      State(id: 3410, name: 'Taitung County', stateCode: 'TTT', cities: [
        City(
            id: 109033,
            name: 'Taitung',
            latitude: '22.88361000',
            longitude: '121.04833000'),
        City(
            id: 109034,
            name: 'Taitung City',
            latitude: '22.75830000',
            longitude: '121.14440000')
      ]),
      State(id: 3419, name: 'Taoyuan City', stateCode: 'TAO', cities: [
        City(
            id: 109006,
            name: 'Daxi',
            latitude: '24.88373000',
            longitude: '121.29043000'),
        City(
            id: 109035,
            name: 'Taoyuan',
            latitude: '24.89500000',
            longitude: '121.24611000'),
        City(
            id: 109036,
            name: 'Taoyuan City',
            latitude: '24.99368000',
            longitude: '121.29696000')
      ]),
      State(id: 3402, name: 'Yilan County', stateCode: 'ILA', cities: [
        City(
            id: 109037,
            name: 'Yilan',
            latitude: '24.54250000',
            longitude: '121.63361000')
      ]),
      State(id: 3416, name: 'Yunlin County', stateCode: 'YUN', cities: [
        City(
            id: 109008,
            name: 'Douliu',
            latitude: '23.70944000',
            longitude: '120.54333000'),
        City(
            id: 109040,
            name: 'Yunlin',
            latitude: '23.70701000',
            longitude: '120.38481000')
      ])
    ]);
