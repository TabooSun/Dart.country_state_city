// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const bhr = Country(
    id: 18,
    name: 'Bahrain',
    iso3: 'BHR',
    iso2: 'BH',
    phoneCode: '973',
    capital: 'Manama',
    currency: 'BHD',
    currencySymbol: r'.د.ب',
    tld: '.bh',
    native: '‏البحرين',
    region: 'Asia',
    subregion: 'Western Asia',
    timezones: [
      Timezone(
          zoneName: 'Asia/Bahrain',
          gmtOffset: 10800,
          gmtOffsetName: 'UTC+03:00',
          abbreviation: 'AST',
          tzName: 'Arabia Standard Time')
    ],
    emoji: '🇧🇭',
    emojiU: 'U+1F1E7 U+1F1ED',
    states: [
      State(id: 1992, name: 'Capital Governorate', stateCode: '13', cities: [
        City(
            id: 9757,
            name: 'Jidd Ḩafş',
            latitude: '26.21861000',
            longitude: '50.54778000'),
        City(
            id: 9760,
            name: 'Manama',
            latitude: '26.22787000',
            longitude: '50.58565000'),
        City(
            id: 9761,
            name: 'Sitrah',
            latitude: '26.15472000',
            longitude: '50.62056000')
      ]),
      State(id: 1996, name: 'Central Governorate', stateCode: '16', cities: [
        City(
            id: 9758,
            name: 'Madīnat Ḩamad',
            latitude: '26.11528000',
            longitude: '50.50694000')
      ]),
      State(id: 1995, name: 'Muharraq Governorate', stateCode: '15', cities: [
        City(
            id: 9754,
            name: 'Al Ḩadd',
            latitude: '26.24556000',
            longitude: '50.65417000'),
        City(
            id: 9753,
            name: 'Al Muharraq',
            latitude: '26.25722000',
            longitude: '50.61194000')
      ]),
      State(
          id: 1994, name: 'Northern Governorate', stateCode: '17', cities: []),
      State(id: 1993, name: 'Southern Governorate', stateCode: '14', cities: [
        City(
            id: 9755,
            name: 'Ar Rifā‘',
            latitude: '26.13000000',
            longitude: '50.55500000'),
        City(
            id: 9756,
            name: 'Dār Kulayb',
            latitude: '26.06861000',
            longitude: '50.50389000'),
        City(
            id: 9759,
            name: 'Madīnat ‘Īsá',
            latitude: '26.17361000',
            longitude: '50.54778000')
      ])
    ]);
