// This file is automatically generated. DO NOT EDIT manually.

import 'package:country_state_city/country_state_city.dart';

const som = Country(
    id: 203,
    name: 'Somalia',
    iso3: 'SOM',
    iso2: 'SO',
    phoneCode: '252',
    capital: 'Mogadishu',
    currency: 'SOS',
    currencySymbol: r'Sh.so.',
    tld: '.so',
    native: 'Soomaaliya',
    region: 'Africa',
    subregion: 'Eastern Africa',
    timezones: [
      Timezone(
          zoneName: 'Africa/Mogadishu',
          gmtOffset: 10800,
          gmtOffsetName: 'UTC+03:00',
          abbreviation: 'EAT',
          tzName: 'East Africa Time')
    ],
    emoji: '🇸🇴',
    emojiU: 'U+1F1F8 U+1F1F4',
    states: [
      State(id: 925, name: 'Awdal Region', stateCode: 'AW', cities: []),
      State(id: 917, name: 'Bakool', stateCode: 'BK', cities: [
        City(
            id: 104815,
            name: 'Tayeeglow',
            latitude: '4.01897000',
            longitude: '44.51111000'),
        City(
            id: 104816,
            name: 'Waajid',
            latitude: '3.80958000',
            longitude: '43.24627000'),
        City(
            id: 104819,
            name: 'Xuddur',
            latitude: '4.12129000',
            longitude: '43.88945000'),
        City(
            id: 104820,
            name: 'Yeed',
            latitude: '4.55000000',
            longitude: '43.03333000')
      ]),
      State(id: 927, name: 'Banaadir', stateCode: 'BN', cities: [
        City(
            id: 104810,
            name: 'Mogadishu',
            latitude: '2.03711000',
            longitude: '45.34375000')
      ]),
      State(id: 930, name: 'Bari', stateCode: 'BR', cities: [
        City(
            id: 104778,
            name: 'Bandarbeyla',
            latitude: '9.49420000',
            longitude: '50.81220000'),
        City(
            id: 104779,
            name: 'Bargaal',
            latitude: '11.28636000',
            longitude: '51.07730000'),
        City(
            id: 104781,
            name: 'Bereeda',
            latitude: '11.87037000',
            longitude: '51.05795000'),
        City(
            id: 104782,
            name: 'Bosaso',
            latitude: '11.28421000',
            longitude: '49.18158000'),
        City(
            id: 104788,
            name: 'Caluula',
            latitude: '11.96611000',
            longitude: '50.75694000'),
        City(
            id: 104800,
            name: 'Iskushuban',
            latitude: '10.28370000',
            longitude: '50.23000000'),
        City(
            id: 104812,
            name: 'Qandala',
            latitude: '11.47197000',
            longitude: '49.87282000')
      ]),
      State(id: 926, name: 'Bay', stateCode: 'BY', cities: [
        City(
            id: 104777,
            name: 'Baidoa',
            latitude: '3.11383000',
            longitude: '43.64980000'),
        City(
            id: 104786,
            name: 'Buurhakaba',
            latitude: '2.80257000',
            longitude: '44.07805000')
      ]),
      State(id: 918, name: 'Galguduud', stateCode: 'GA', cities: [
        City(
            id: 104790,
            name: 'Ceelbuur',
            latitude: '4.68501000',
            longitude: '46.61760000'),
        City(
            id: 104791,
            name: 'Ceeldheer',
            latitude: '3.84878000',
            longitude: '47.18064000'),
        City(
            id: 104793,
            name: 'Dhuusamarreeb',
            latitude: '5.53597000',
            longitude: '46.38666000')
      ]),
      State(id: 928, name: 'Gedo', stateCode: 'GE', cities: [
        City(
            id: 104776,
            name: 'Baardheere',
            latitude: '2.34464000',
            longitude: '42.27644000'),
        City(
            id: 104797,
            name: 'Garbahaarrey',
            latitude: '3.32892000',
            longitude: '42.22091000'),
        City(
            id: 104807,
            name: 'Luuq',
            latitude: '3.80315000',
            longitude: '42.54417000')
      ]),
      State(id: 915, name: 'Hiran', stateCode: 'HI', cities: [
        City(
            id: 104780,
            name: 'Beledweyne',
            latitude: '4.73583000',
            longitude: '45.20361000'),
        City(
            id: 104784,
            name: 'Buulobarde',
            latitude: '3.85375000',
            longitude: '45.56744000'),
        City(
            id: 104801,
            name: 'Jalalaqsi',
            latitude: '3.37660000',
            longitude: '45.59960000')
      ]),
      State(id: 924, name: 'Lower Juba', stateCode: 'JH', cities: [
        City(
            id: 104785,
            name: 'Buur Gaabo',
            latitude: '-1.21917000',
            longitude: '41.83725000'),
        City(
            id: 104802,
            name: 'Jamaame',
            latitude: '0.06968000',
            longitude: '42.74497000'),
        City(
            id: 104805,
            name: 'Kismayo',
            latitude: '-0.35817000',
            longitude: '42.54536000')
      ]),
      State(id: 921, name: 'Lower Shebelle', stateCode: 'SH', cities: [
        City(
            id: 104775,
            name: 'Afgooye',
            latitude: '2.13810000',
            longitude: '45.12120000'),
        City(
            id: 104809,
            name: 'Marka',
            latitude: '1.71594000',
            longitude: '44.77166000'),
        City(
            id: 104813,
            name: 'Qoryooley',
            latitude: '1.78784000',
            longitude: '44.52999000'),
        City(
            id: 104817,
            name: 'Wanlaweyn',
            latitude: '2.61850000',
            longitude: '44.89380000')
      ]),
      State(id: 922, name: 'Middle Juba', stateCode: 'JD', cities: [
        City(
            id: 104794,
            name: 'Dujuuma',
            latitude: '1.25311000',
            longitude: '42.57377000'),
        City(
            id: 104804,
            name: 'Jilib',
            latitude: '0.48829000',
            longitude: '42.78535000'),
        City(
            id: 104814,
            name: 'Saacow',
            latitude: '1.62794000',
            longitude: '42.44067000')
      ]),
      State(id: 923, name: 'Middle Shebelle', stateCode: 'SD', cities: [
        City(
            id: 104787,
            name: 'Cadale',
            latitude: '2.76030000',
            longitude: '46.32220000'),
        City(
            id: 104803,
            name: 'Jawhar',
            latitude: '2.78087000',
            longitude: '45.50048000'),
        City(
            id: 104808,
            name: 'Mahaddayweyne',
            latitude: '2.97260000',
            longitude: '45.53470000')
      ]),
      State(id: 916, name: 'Mudug', stateCode: 'MU', cities: [
        City(
            id: 104796,
            name: 'Gaalkacyo',
            latitude: '6.76972000',
            longitude: '47.43083000'),
        City(
            id: 104799,
            name: 'Hobyo',
            latitude: '5.35050000',
            longitude: '48.52680000'),
        City(
            id: 104818,
            name: 'Xarardheere',
            latitude: '4.65440000',
            longitude: '47.85750000')
      ]),
      State(id: 920, name: 'Nugal', stateCode: 'NU', cities: [
        City(
            id: 104795,
            name: 'Eyl',
            latitude: '7.98030000',
            longitude: '49.81640000'),
        City(
            id: 104798,
            name: 'Garoowe',
            latitude: '8.40207000',
            longitude: '48.48284000')
      ]),
      State(id: 919, name: 'Sanaag Region', stateCode: 'SA', cities: [
        City(
            id: 104792,
            name: 'Ceerigaabo',
            latitude: '10.61616000',
            longitude: '47.36795000'),
        City(
            id: 104806,
            name: 'Las Khorey',
            latitude: '11.15950000',
            longitude: '48.19670000')
      ]),
      State(id: 929, name: 'Togdheer Region', stateCode: 'TO', cities: [
        City(
            id: 104783,
            name: 'Burao',
            latitude: '9.52213000',
            longitude: '45.53363000'),
        City(
            id: 104789,
            name: 'Ceek',
            latitude: '8.99907000',
            longitude: '45.35824000'),
        City(
            id: 104811,
            name: 'Oodweyne',
            latitude: '9.40920000',
            longitude: '45.06397000')
      ])
    ]);
